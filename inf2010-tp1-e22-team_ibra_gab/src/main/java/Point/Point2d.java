...
public class Point2d extends AbstractPoint {
    ...
    public Point2d translate(Double[] translateVector) {
        Double[] newVector = new Double[]{translateVector[0] + X(), translateVector[1] + Y()};
    ...
    public Point2d rotate(Double angle) {
        double rotationX = vector[X]*Math.cos(angle) + vector[Y]*-1*Math.sin(angle);
        double rotationY = vector[Y]*Math.cos(angle) + vector[X]*Math.sin(angle);
        vector[X] = rotationX;
        vector[Y] = rotationY;
        return this;
    }
    ...
    public Point2d clone() {
        return new Point2d(new Double[]{this.X(),this.Y()});
    }
}
